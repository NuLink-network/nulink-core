...
from nulink.blockchain.eth.agents import ContractAgency, NulinkTokenAgent, PREApplicationAgent
from nulink.blockchain.eth.deployers import (
    PREApplicationDeployer,
    SubscriptionManagerDeployer, NuLinkTokenDeployer
)
from nulink.blockchain.eth.interfaces import BlockchainInterfaceFactory
from nulink.blockchain.eth.registry import InMemoryContractRegistry, LocalContractRegistry
...
from nulink.config.characters import (
    AliceConfiguration,
    BobConfiguration,
    UrsulaConfiguration
)
...
def temp_dir_path():
    temp_dir = tempfile.TemporaryDirectory(prefix='nulink-test-')
...
def certificates_tempdir():
    custom_filepath = '/tmp/nulink-test-certificates-'
...
def _make_agency(test_registry, token_economics, deployer_transacting_power, threshold_staking):
    ...
    token_deployer = NuLinkTokenDeployer(economics=token_economics, registry=test_registry)
...
def funded_blockchain(testerchain, agency, application_economics, test_registry):
    ...
    # Free Tokens!!!
    token_airdrop(token_agent=NulinkTokenAgent(registry=test_registry),
...
def stakeholder_config_file_location():
    path = Path('/', 'tmp', 'nulink-test-stakeholder.json')
    if path.exists():
        path.unlink()
    yield path
    if path.exists():
...
def software_stakeholder(testerchain, agency, stakeholder_config_file_location, test_registry):
    ...
    token_agent.transfer(amount=NLK(200_000, 'NLK').to_units(),
                         transacting_power=transacting_power,
