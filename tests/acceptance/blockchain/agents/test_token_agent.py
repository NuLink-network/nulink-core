...
def agent(testerchain, test_registry) -> NulinkTokenAgent:
...
def test_token_properties(agent):
    ...
    # Internal
    assert 'NuLink' == agent.contract.functions.name().call()
    assert 18 == agent.contract.functions.decimals().call()
    assert 'NLK' == agent.contract.functions.symbol().call()
    ...
    assert agent.contract_name == NulinkTokenAgent.contract_name
