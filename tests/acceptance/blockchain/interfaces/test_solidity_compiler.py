...
from nulink.blockchain.eth.sol.compile.constants import DEFAULT_VERSION_STRING, TEST_MULTIVERSION_CONTRACTS, \
    TEST_SOLIDITY_SOURCE_ROOT, SOLIDITY_SOURCE_ROOT
from nulink.blockchain.eth.sol.compile.types import SourceBundle


def test_nulink_contract_compiled(testerchain, test_registry):
    """Ensure that solidity smart contacts are available, post-compile."""
    origin, *everybody_else = testerchain.client.accounts

    token_contract_identifier = NuLinkTokenDeployer(registry=test_registry).contract_name
