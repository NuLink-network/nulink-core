...
from nulink.config.constants import APP_DIR, DEFAULT_CONFIG_ROOT, NULINK_ENVVAR_KEYSTORE_PASSWORD, TEMPORARY_DOMAIN
...
def test_initialize_ursula_defaults(click_runner, mocker, tmpdir):
    ...
    result = click_runner.invoke(nulink_cli, init_args, input=user_input, catch_exceptions=False)
...
def test_initialize_custom_configuration_root(click_runner, custom_filepath: Path):
    ...
    assert "nulink ursula run" in result.output, 'Help message is missing suggested command'
...
def test_ursula_view_configuration(custom_filepath: Path, click_runner, nominal_federated_configuration_fields):
    ...
    result = click_runner.invoke(nulink_cli, view_args,
...
def test_run_federated_ursula_from_config_file(custom_filepath: Path, click_runner):
    ...
    result = click_runner.invoke(nulink_cli, run_args,
                                 input='{}\nY\n'.format(INSECURE_DEVELOPMENT_PASSWORD),
                                 catch_exceptions=False)
...
def test_ursula_save_metadata(click_runner, custom_filepath):
    ...
    result = click_runner.invoke(nulink_cli, save_metadata_args, catch_exceptions=False)
...
def test_ursula_destroy_configuration(custom_filepath, click_runner):
    ...
    result = click_runner.invoke(nulink_cli, destruction_args,
                                 input='Y\n'.format(INSECURE_DEVELOPMENT_PASSWORD),
                                 catch_exceptions=False,
    ...
    assert custom_filepath.is_dir(), 'NuLink files no longer exist'  # ... but not NuLink ...
