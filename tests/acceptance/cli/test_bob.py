...
from nulink.cli.commands.bob import BobCharacterOptions
from nulink.cli.literature import SUCCESSFUL_DESTRUCTION, COLLECT_NULINK_PASSWORD
...
from nulink.control.emitters import JSONRPCStdoutEmitter
from nulink.crypto.powers import SigningPower
from nulink.policy.identity import Card
from nulink.utilities.logging import GlobalLoggerSettings, Logger
...
@mock.patch('nulink.config.characters.BobConfiguration.default_filepath', return_value='/non/existent/file')
def test_missing_configuration_file(default_filepath_mock, click_runner):
    ...
    assert "nulink bob init" in result.output
...
def test_initialize_bob_with_custom_configuration_root(click_runner, custom_filepath: Path):
    ...
    assert "nulink bob run" in result.output, 'Help message is missing suggested command'
...
def test_bob_retrieve_and_decrypt(click_runner,
    ...
    envvars = {'NULINK_KEYSTORE_PASSWORD': INSECURE_DEVELOPMENT_PASSWORD}
    ...
    retrieve_response = click_runner.invoke(nulink_cli,
                                            retrieve_args,
