...
from nulink.config.characters import AliceConfiguration, BobConfiguration
from nulink.config.constants import NULINK_ENVVAR_KEYSTORE_PASSWORD, TEMPORARY_DOMAIN, \
    NULINK_ENVVAR_ALICE_ETH_PASSWORD, NULINK_ENVVAR_BOB_ETH_PASSWORD
...
def run_entire_cli_lifecycle(click_runner,
    ...
    """
    Scene 1: Alice Installs nulink to a custom filepath and examines her configuration
    """
    ...
    alice_init_response = click_runner.invoke(nulink_cli, alice_init_args, catch_exceptions=False, env=envvars)
    ...
    alice_view_result = click_runner.invoke(nulink_cli,
                                            alice_view_args,
    ...
    """
    Scene 2: Bob installs nulink, examines his configuration and expresses his
    ...
    bob_init_response = click_runner.invoke(nulink_cli, bob_init_args, catch_exceptions=False, env=envvars)
    ...
    bob_view_result = click_runner.invoke(nulink_cli, bob_view_args, catch_exceptions=False, env=envvars)
    ...
    derive_response = click_runner.invoke(nulink_cli, derive_args, catch_exceptions=False, env=envvars)
    ...
    def enrico_encrypts():
        ...
        encrypt_result = click_runner.invoke(nulink_cli, enrico_args, catch_exceptions=False, env=envvars)
    ...
    def _alice_decrypts(encrypt_result):
        ...
        decrypt_response_fail = click_runner.invoke(nulink_cli, decrypt_args[0:7], catch_exceptions=False,
                                                    env=envvars)
        assert decrypt_response_fail.exit_code == 2

        decrypt_response = click_runner.invoke(nulink_cli, decrypt_args, catch_exceptions=False, env=envvars)
    ...
    def _grant(teacher_uri):
        ...
        grant_result = click_runner.invoke(nulink_cli, grant_args, catch_exceptions=False, env=envvars)
    ...
    def _bob_retrieves(_grant_result):
        ...
        retrieve_response = click_runner.invoke(nulink_cli, retrieve_args, catch_exceptions=False, env=envvars)
