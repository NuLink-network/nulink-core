...
def test_create_token(testerchain, deploy_contract):
    ...
    token, txhash = deploy_contract('NuLinkToken', TOTAL_SUPPLY)
    ...
    # Basic properties
    assert 'NuLink' == token.functions.name().call()
    assert 18 == token.functions.decimals().call()
    assert 'NLK' == token.functions.symbol().call()
...
def test_approve_and_call(testerchain, deploy_contract):
    ...
    token, _ = deploy_contract('NuLinkToken', TOTAL_SUPPLY)
