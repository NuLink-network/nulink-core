...
from nulink.blockchain.eth.decorators import InvalidChecksumAddress
...
from nulink.crypto import passwords
from nulink.crypto.keystore import Keystore
from nulink.crypto.passwords import SecretBoxAuthenticationError
...
def test_get_password_from_prompt_cli_action(mocker, mock_stdin, confirm, capsys):
    ...
    test_envvar = 'NULINK_TEST_ENVVAR'
...
@pytest.mark.parametrize('confirm', (True, False))
def test_get_nulink_password(mock_stdin, mock_account, confirm, capsys):
    mock_stdin.password(INSECURE_DEVELOPMENT_PASSWORD, confirm=confirm)
    result = get_nulink_password(emitter=StdoutEmitter(), confirm=confirm)
    assert result == INSECURE_DEVELOPMENT_PASSWORD
    assert mock_stdin.empty()
    captured = capsys.readouterr()
    assert COLLECT_NULINK_PASSWORD in captured.out
    if confirm:
        prompt = COLLECT_NULINK_PASSWORD + f" ({Keystore._MINIMUM_PASSWORD_LENGTH} character minimum)"
        assert prompt in captured.out


def test_unlock_nulink_keystore_invalid_password(mocker,
                                                   test_emitter,
    ...
    with pytest.raises(Keystore.AuthenticationFailed):
        unlock_nulink_keystore(emitter=test_emitter,
...
def test_unlock_nulink_keystore_dev_mode(mocker, test_emitter, capsys, alice_blockchain_test_config, tmpdir):
    ...
    result = unlock_nulink_keystore(emitter=test_emitter,
...
def test_unlock_nulink_keystore(mocker,
                                  test_emitter,
                                  capsys,
                                  alice_blockchain_test_config,
                                  patch_keystore,
