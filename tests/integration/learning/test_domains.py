...
def test_learner_learns_about_domains_separately(lonely_ursula_maker, caplog):
    hero_learner, other_first_domain_learner = lonely_ursula_maker(domain="nulink1.test_suite", quantity=2)
    _nobody = lonely_ursula_maker(domain="nulink1.test_suite", quantity=1).pop()
    other_first_domain_learner.remember_node(_nobody)

    second_domain_learners = lonely_ursula_maker(domain="nulink2.test_suite", know_each_other=True, quantity=3)
    ...
    new_first_domain_learner = lonely_ursula_maker(domain="nulink1.test_suite", quantity=1).pop()
    _new_second_domain_learner = lonely_ursula_maker(domain="nulink2.test_suite", quantity=1).pop()
...
def test_learner_ignores_stored_nodes_from_other_domains(lonely_ursula_maker, tmpdir):
    ...
    # This is how heco testnet nodes initially made their way into mainnet fleet states.
