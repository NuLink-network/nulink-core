...
from nulink.network.server import make_rest_app
from tests.mock.serials import good_serials

mock_cert_storage = patch("nulink.config.storages.ForgetfulNodeStorage.store_node_certificate",
                          new=lambda *args, **kwargs: "this_might_normally_be_a_filepath")
mock_message_verification = patch('nulink.characters.lawful.Alice.verify_from', new=lambda *args, **kwargs: None)
...
mock_keep_learning = patch('nulink.network.nodes.Learner.keep_learning_about_nodes', new=fake_keep_learning)

mock_record_fleet_state = patch("nulink.acumen.perception.FleetSensor.record_fleet_state",
...
mock_cert_loading = patch("nulink.network.nodes.load_der_x509_certificate",
...
mock_cert_generation = patch("nulink.crypto.tls.generate_self_signed_certificate", new=do_not_create_cert)
mock_rest_app_creation = patch("nulink.characters.lawful.make_rest_app",
                               new=NotARestApp.create_with_not_a_datastore)

mock_remember_node = patch("nulink.characters.lawful.Ursula.remember_node", new=simple_remember)
mock_verify_node = patch("nulink.characters.lawful.Ursula.verify_node", new=VerificationTracker.fake_verify_node)

mock_metadata_validation = patch("nulink.network.nodes.Teacher.validate_metadata",
                                 new=VerificationTracker.fake_verify_metadata)


@contextmanager
def mock_secret_source(*args, **kwargs):
    with patch("nulink.crypto.keypairs.Keypair._private_key_source", new=lambda *args, **kwargs: NotAPrivateKey()):
        with patch("nulink.crypto.keypairs.Signer", new=lambda *args, **kwargs: NotASigner(*args, **kwargs)):
            yield
    NotAPublicKey.reset()
