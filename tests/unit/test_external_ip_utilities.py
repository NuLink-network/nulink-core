...
from nulink.crypto.tls import generate_self_signed_certificate
...
from nulink.utilities.networking import (
...
def mock_requests(mocker):
    ...
    make_request = 'nulink.utilities.networking._request'
...
def mock_client(mocker):
    ...
    mocker.patch.object(NulinkMiddlewareClient, 'get_certificate', return_value=(cert, Path()))
    yield mocker.patch.object(NulinkMiddlewareClient, 'invoke_method', return_value=Dummy.GoodResponse)
...
def test_get_external_ip_cascade_failure(mocker, mock_requests):
    first = mocker.patch('nulink.utilities.networking.get_external_ip_from_known_nodes', return_value=None)
    second = mocker.patch('nulink.utilities.networking.get_external_ip_from_default_teacher', return_value=None)
    third = mocker.patch('nulink.utilities.networking.get_external_ip_from_centralized_source', return_value=None)
