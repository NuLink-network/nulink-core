...
def test_but_nulink_logger_is_cool_with_that(capsys):
    nulink_logger = Logger('nulink-logger', observer=naive_print_observer)

    # Normal strings are logged normally
    for string in acceptable_strings:
        nulink_logger.info(string)
        captured = capsys.readouterr()
        assert string == captured.out
        assert not captured.err
...
def test_even_nulink_json_logger_is_cool():

    nulink_logger = Logger('nulink-logger-json')

    # Normal strings are logged normally
    for string in acceptable_strings:
        file = StringIO()
        nulink_logger.observer = get_json_observer_for_file(file)
        nulink_logger.info(string)
        logged_event = file.getvalue()
        assert '"log_level": {"name": "info"' in logged_event
