...
from nulink.cli.actions.configure import get_config_filepaths
...
from nulink.config.constants import NULINK_ENVVAR_OPERATOR_ADDRESS, DEFAULT_CONFIG_ROOT
...
def select_client_account(emitter,
    ...
    Note: Showing ETH and/or NLK balances, causes an eager blockchain connection.
    """
    ...
    for index, account in enumerated_accounts.items():
        ...
        if show_nu_balance:
            ...
            token_balance = NLK.from_units(token_agent.get_balance(account, registry))
...
def select_network(emitter: StdoutEmitter, network_type: str, message: Optional[str] = None) -> str:
    ...
    elif network_type == NetworksInventory.HECO:
        network_list = NetworksInventory.HECO_NETWORKS
    # elif network_type == NetworksInventory.POLYGON:
    #     network_list = NetworksInventory.POLY_NETWORKS
    else:
        raise(ValueError("Network type must be either 'eth' or 'heco'"))
...
def select_config_file(emitter: StdoutEmitter,
    ...
    checksum_address = checksum_address or os.environ.get(NULINK_ENVVAR_OPERATOR_ADDRESS, None)  # TODO: Deprecate operator_address in favor of checksum_address
