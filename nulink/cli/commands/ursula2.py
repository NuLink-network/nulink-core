...
from nulink.cli.actions.auth import get_client_password, get_nulink_password, recover_keystore
...
from nulink.cli.actions.configure import forget as forget_nodes, perform_startup_ip_check
from nulink.cli.actions.select import select_client_account, select_config_file, select_network
...
class UrsulaConfigOptions:
    ...
    def generate_config(self, emitter, config_root, force, key_material):
        ...
        return UrsulaConfiguration.generate(password=get_nulink_password(emitter=emitter, confirm=True),
    ...
    def create_character(self, emitter, config_file, json_ipc, load_seednodes=True, verify_node_bonding=True):
        ...
        if password_required:
            __password = get_client_password(checksum_address=ursula_config.operator_address,
                                             envvar=NULINK_ENVVAR_OPERATOR_ETH_PASSWORD)
...
def init(general_config, config_options, force, config_root, key_material):
    ...
    if not config_options.federated_only and not config_options.payment_network:
        config_options.payment_network = select_network(emitter, message="Select Payment Network", network_type=NetworksInventory.HECO)
...
def run(general_config, character_options, config_file, interactive, dry_run, prometheus, metrics_port,
    ...
    #     from nulink.utilities.prometheus.metrics import PrometheusMetricsConfig
...
if __name__ == '__main__':
    ...
    #     '--payment-provider', 'https://bsc-testnet.blockpi.network/v1/rpc/public',
