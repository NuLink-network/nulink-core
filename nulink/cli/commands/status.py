...
from nulink.blockchain.eth.constants import AVERAGE_BLOCK_TIME_IN_SECONDS
...
from nulink.cli.painting.status import paint_contract_status
from nulink.cli.utils import (
...
group_registry_options = group_options(
    ...
    eth_provider_uri=option_eth_provider_uri(default=os.environ.get(NULINK_ENVVAR_ETH_PROVIDER_URI)),
)
...
@click.group()
def status():
    """Echo a snapshot of live NuLink Network metadata."""
...
def network(general_config, registry_options):
    """Overall information of the NuLink Network."""
...
@click.option('--legacy', help="Events related to the NuLink Network prior to the merge to Threshold Network", is_flag=True)
def events(general_config, registry_options, contract_name, from_block, to_block, event_name, csv, csv_file, event_filters, legacy):
    """Show events associated with NuLink contracts."""
