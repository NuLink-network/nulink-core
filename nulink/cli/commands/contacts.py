...
from nulink.cli.actions.select import select_card
from nulink.cli.options import option_force
from nulink.cli.painting.policies import paint_single_card, paint_cards
from nulink.cli.types import EXISTING_READABLE_FILE, UMBRAL_PUBLIC_KEY_HEX
...
def _list():
    ...
    if not card_filepaths:
        emitter.error(f'No cards found at {Card.CARD_DIR}.  '
                      f"To create one run 'nulink {contacts.name} {create.name}'.")
    cards = list()
