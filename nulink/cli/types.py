...
from nulink.policy.payment import PAYMENT_METHODS
from nulink.utilities.networking import validate_operator_ip, InvalidOperatorIP
...
class NuLinkNetworkName(click.ParamType):
    name = 'nulink_network_name'
    ...
    def convert(self, value, param, ctx):
        if self.validate:
            ...
            if network not in NetworksInventory.NETWORKS:
                self.fail(f"'{value}' is not a NuLink Network. Valid options are: {list(NetworksInventory.NETWORKS)}")
            else:
                return network
        else:
            return value
    ...
    def convert(self, value, param, ctx):
        if self.validate:
            try:
                ...
                self.fail(f"'{value}' is not a valid nulink public key.")
        return value
...
__min_authorization = NLK.from_units(Economics._default_min_authorization).to_tokens()
