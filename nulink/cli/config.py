...
from nulink.cli.utils import get_env_bool
from nulink.config.constants import DEFAULT_CONFIG_ROOT, NULINK_SENTRY_ENDPOINT
...
class GroupGeneralConfig:
    ...
    sentry_endpoint = os.environ.get("NULINK_SENTRY_DSN", NULINK_SENTRY_ENDPOINT)
    log_to_sentry = get_env_bool("NULINK_SENTRY_LOGS", False)
    log_to_file = get_env_bool("NULINK_FILE_LOGS", True)
...
group_general_config = group_options(
    ...
    file_logs=click.option(
        ...
        help="Enable/disable logging to file. Defaults to NULINK_FILE_LOGS, or to `--file-logs` if it is not set.",
        default=None),

    sentry_logs=click.option(
        '--sentry-logs/--no-sentry-logs',
        help="Enable/disable logging to Sentry. Defaults to NULINK_SENTRY_LOGS, or to `--sentry-logs` if it is not set.",
        default=None),
