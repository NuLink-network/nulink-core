...
from nulink.config import constants
from nulink.config.storages import (
...
from nulink.crypto.powers import CryptoPower, CryptoPowerUp
...
class CharacterConfiguration(BaseConfiguration):
    ...
    TEMP_CONFIGURATION_DIR_PREFIX = 'tmp-nulink'
    SIGNER_ENVVAR = None
    ...
    def initialize(self, password: str, key_material: Optional[bytes] = None) -> str:
        ...
        # Success
        message = "Created nulink installation files at {}".format(self.config_root)
        self.log.debug(message)
