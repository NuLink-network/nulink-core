...
from nulink.blockchain.eth.clients import PUBLIC_CHAINS
from nulink.blockchain.eth.networks import NetworksInventory
from nulink.config.constants import DEFAULT_CONFIG_ROOT, DEPLOY_DIR, NULINK_ENVVAR_KEYSTORE_PASSWORD, \
...
class BaseCloudNodeConfigurator:
    ...
    def __init__(self,  # TODO: Add type annotations
        ...
        self.output_capture = {'worker address': [], 'rest url': [], 'nulink version': [], 'nickname': []}
        ...
        self.config['nulink_image'] = nulink_image or self.config.get('nulink_image') or 'nulink/nulink:latest'
    ...
    def update_generate_inventory(self, node_names, **kwargs):
        ...
        if not nodes:
            raise KeyError(f"No hosts matched the supplied names: {node_names}.  Try `nulink cloudworkers list-hosts`")

        defaults = {
            'envvars':
                [
                    (NULINK_ENVVAR_KEYSTORE_PASSWORD,  self.config['keystorepassword']),
                    (NULINK_ENVVAR_WORKER_ETH_PASSWORD, self.config['ethpassword']),
                ],
            'cliargs': [
            ]
        }
    ...
    def deploy_nulink_on_existing_nodes(self, node_names, wipe_nulink=False):
        ...
        self.update_generate_inventory(node_names, wipe_nulink=wipe_nulink)
    ...
    def update_nulink_on_existing_nodes(self, node_names):
...
class AWSNodeConfigurator(BaseCloudNodeConfigurator):
    ...
    def _configure_path_to_internet(self):
        ...
        if not self.config.get('SecurityGroup'):
            securitygroupdata = self.ec2Client.create_security_group(GroupName=f'Ursula-{self.namespace_network}', Description='ssh and NuLink ports', VpcId=self.config['Vpc'])
...
class GenericConfigurator(BaseCloudNodeConfigurator):
    ...
    def _write_config(self):
        if not self.config_path.exists() and self.action not in self.NAMESSPACE_CREATE_ACTIONS:
            raise AttributeError(f"Namespace/config '{self.namespace}' does not exist. Show existing namespaces: `nulink cloudworkers list-namespaces` or create a namespace: `nulink cloudworkers create`")
