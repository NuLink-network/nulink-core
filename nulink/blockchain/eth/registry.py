...
from nulink.blockchain.eth import CONTRACT_REGISTRY_BASE
...
class GithubRegistrySource(CanonicalRegistrySource):
    ...
    _PUBLICATION_REPO = "NuLink-network/nulink"
    ...
    def get_publication_endpoint(self) -> str:
        url = f"{self._BASE_URL}/development/nulink/blockchain/eth/contract_registry/{self.network}/{self.registry_name}"
        return url
...
class RegistrySourceManager:
    ...
    # at present， how no the reomote source, so we adjust it
    # _FALLBACK_CHAIN = _REMOTE_SOURCES + _LOCAL_SOURCES
    _FALLBACK_CHAIN = _LOCAL_SOURCES
...
class BaseContractRegistry(ABC):
    ...
    WARNING: Unless you are developing NuLink, you most likely won't ever need
    to use this.
    """
    ...
    def enroll(self, contract_name, contract_address, contract_abi, contract_version) -> None:
        ...
        Note: Unless you are developing NuLink, you most likely won't ever
        need to use this.
        """
