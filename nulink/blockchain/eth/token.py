...
class ERC20:
    ...
    Decimal: nlk = NLK.from_tokens(Decimal('100.50'))
    Int: nlk = NLK.from_units(15000000000000000000000)

    Token quantity is stored internally as an int in the smallest denomination,
    and all arithmetic operations use this value.

    Using float inputs to this class to represent amounts of NLK is supported but not recommended,
    ...
    def to_tokens(self) -> Decimal:
        """Returns a decimal value of NLK"""
...
class WorkTrackerBase:
    ...
    def __fire_replacement_commitment(self, current_block_number: int, tx_firing_block_number: int) -> None:
        replacement_txhash = self._fire_commitment()  # replace # operator用来确认与staker正确绑定的 交易,看子类代码
...
class WorkTracker(WorkTrackerBase):
    """
        andi comment: worker 质押的操作者确认与质押者正确绑定的操作
    """
