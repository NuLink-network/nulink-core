...
from nulink.blockchain.eth.constants import DISPATCHER_CONTRACT_NAME, STAKING_ESCROW_CONTRACT_NAME, NULL_ADDRESS
...
class NuLinkTokenDeployer(BaseContractDeployer):

    agency = NulinkTokenAgent
    contract_name = agency.contract_name
    deployment_steps = ('contract_deployment', )
    _upgradeable = False
    _ownable = False

    TOTAL_SUPPLY = NLK(1_000_000_000, 'NLK').to_units()
    ...
    def deploy(self,
               ...
               **overrides) -> dict:
        """
        Deploy and publish the NuLink Token contract
...
class StakingEscrowDeployer(BaseContractDeployer, UpgradeableContractMixin, OwnableContractMixin):
    """
    Deploys the StakingEscrow ethereum contract to the blockchain.  Depends on NulinkTokenAgent
    """
    ...
    STUB_MIN_ALLOWED_TOKENS = NLK(15_000, 'NLK').to_units()
    STUB_MAX_ALLOWED_TOKENS = NLK(30_000_000, 'NLK').to_units()
    ...
    def __init__(self,
        ...
        token_contract_name = NuLinkTokenDeployer.contract_name
