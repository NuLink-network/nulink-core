...
class FileLogObserver:
    ...
    def __call__(self, event: LogEvent) -> None:
        # add write_file by andi to fix I/O operation on closed file
        def write_file():
            """
            Write event to file.
        ...
        try:
            ...
            # add reopen() by andi to fix I/O operation on closed file
            if ("I/O operation on closed file".lower() in str(e).lower()):
                self._outFile.reopen()
                write_file()
            else:
                raise
