...
# Disable logging
export NULINK_SENTRY_LOGS=0
export NULINK_FILE_LOGS=0
...
nulink ursula run --debug --dev --federated-only --teacher localhost:11500 --rest-port 11501 > /tmp/ursulas-logs/ursula-11501.txt 2>&1 &
sleep 1

# Connect Node #3 to the local Fleet
echo "Starting Ursula #3..."
nulink ursula run --debug --dev --federated-only --teacher localhost:11500 --rest-port 11502 > /tmp/ursulas-logs/ursula-11502.txt 2>&1 &
