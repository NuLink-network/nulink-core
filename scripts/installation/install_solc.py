...
This script depends on `nulink.blockchain.eth.sol` submodule and `py-solc-x` library 
but otherwise does not require installation of `nulink` or its dependencies. 
"""
...
def get_solc_config_path() -> Path:
    ...
    nulink = Path(__file__).parent.parent.parent.resolve() / 'nulink'
    config_path = nulink / 'blockchain' / 'eth' / 'sol' / '__conf__.py'
    return config_path
...
def get_solc_version() -> str:
    ...
    try:
        ...
        try:
            version = os.environ['NULINK_SOLIDITY_VERSION']  # 2
        except KeyError:
