...
This guide is an example of NuLink's decentralized network allowing Alice to share a secret
with Bob using the PRE Application on the Threshold Network via the ``nulink`` CLI. It is analogous to the ``python`` example
...
.. note::
    ...
    with updated `geth` (``~/.ethereum/rinkeby/geth.ipc``) and `seed` URI (``https://to/be/supplied``).
...
.. code::
    ...
    # Create a new Alice
    (nulink)$ nulink alice init --eth-provider <ETH PROVIDER URI> --signer <SIGNER URI> --network heco_testnet

    # Create a new Bob
    (nulink)$ nulink bob init --eth-provider <ETH PROVIDER URI> --signer <SIGNER URI> --network heco_testnet

Replace ``<YOUR ETH PROVIDER URI>`` with a valid node web3 node provider string on the heco testnet ethereum network, for example:
...
.. code::
    ...
    (nulink)$ echo UGVhY2UgYXQgRGF3bg== | base64 -d
    Peace at Dawn
