...
#. `Porter`_ - the "Infura for NuLink" - a web-based service for interacting with the network PRE Application protocol.
#. :ref:`nulink-ts-library` - middleware javascript layer that ties the underlying cryptography ``rust-umbral`` layer to the use of Porter.
...
Overview
--------
NuLink Porter can be described as the *“Infura for NuLink”*. Porter is a web-based service that performs
nulink-based protocol operations on behalf of applications.

Its goal is to simplify and abstract the complexities surrounding the nulink protocol to negate the need for
applications to interact with it via a python client. Porter introduces the nulink protocol to cross-platform
...
Run Porter within Docker without acquiring or installing the ``nulink`` codebase.

#. Get the latest ``nulink`` image:

   .. code:: bash

       $ docker pull nulink/porter:latest
...
#. :ref:`acquire_codebase`. There is no need
   to install ``nulink`` after acquiring the codebase since Docker will be used.
...
#. Set the required environment variables:
   ...
   * Network Name environment variable
     ...
     .. code:: bash
         ...
         $ export NULINK_NETWORK=<NETWORK NAME>
...
Install ``nulink`` - see :doc:`/references/installation`.

For a full list of CLI options, run:

.. code:: console

  $ nulink porter run --help
...
* Run Porter service
  ...
  * Run via HTTP
    ...
    .. code:: console
        ...
        $ nulink porter run --eth-provider <YOUR WEB3 PROVIDER URI> --network <NETWORK NAME>
  ...
  * Run via HTTPS
    ...
    .. code:: console
        ...
        $ nulink porter run --eth-provider <YOUR WEB3 PROVIDER URI> --network <NETWORK NAME> --tls-key-filepath <TLS KEY FILEPATH> --tls-certificate-filepath <CERT FILEPATH>
    ...
    .. code:: console
        ...
        $ nulink porter run --eth-provider <YOUR WEB3 PROVIDER URI> --network <NETWORK NAME> --tls-key-filepath <TLS KEY FILEPATH> --tls-certificate-filepath <CERT FILEPATH> --allow-origins ".*\.example\.com$"
    ...
    .. code:: console
        ...
        $ nulink porter run --eth-provider <YOUR WEB3 PROVIDER URI> --network <NETWORK NAME> --tls-key-filepath <TLS KEY FILEPATH> --tls-certificate-filepath <CERT FILEPATH> --allow-origins ".*\.example\.com$" --basic-auth-filepath <HTPASSWD FILE>
...
#. Set the required environment variables:
   ...
   * *(Optional)* The CORS configuration is set in the ``nulink/deploy/docker/porter/nginx/porter.local_location`` file.
...
`nulink-ts <https://github.com/NuLink-network/nulink-ts>`_ is a typescript middleware layer implementation of the ``nulink`` python library. It abstracts
